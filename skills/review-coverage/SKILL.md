...
1. Tool availability summary (Go version, test outcome, whether `coverage.out` was produced, prior baseline location if any)
2. Per-package coverage table (with delta column when baseline existed)
3. Uncovered functions table (grouped by package, sorted by severity)
