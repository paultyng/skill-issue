...
Do **not** use when the bug is obviously in the most recent commit (just inspect it), or when the reproducer is too fragile/flaky to drive an automated bisect (fix the flake first).
...
- **Merges in the range.** `git bisect` handles them — it linearizes via the commit graph. No special action.
