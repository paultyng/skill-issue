...
- Use RFC 2119 key words (`MUST` / `SHOULD` / `MAY`) for requirement levels, sparingly and only for genuine requirements. See the `normative-language` rule. For a requirement that is also frequently violated, add a short "Frequently missed:" lead-in.
